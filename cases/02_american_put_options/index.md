...
where the expectation is taken under the risk-neutral probability measure, $\mathcal{Q}_{t,T}$ is the set of all possible stopping times $\tau \in [t,T]$, $T$ is the maturity time and $g(x) = (K-x)\_+$ with $K$ the strike price and $\mathcal{F}_W(t)$ the filtration generated by Brownian motion $W(t)$. Since the underlying stock price is a Markov process (1.0) simplifies to
